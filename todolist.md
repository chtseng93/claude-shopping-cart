...
| 2026-06-20 | Material Symbols 修復（完成）| 補上 `.material-symbols-outlined` 缺漏的 `font-family` 宣告，解決全站圖示（star、content_copy、favorite、shopping_bag 等）顯示為文字的問題；新增本地字型 `frontend/public/fonts/MaterialSymbolsOutlined.woff2` |
| 2026-06-20 | Demo2 重錄（完成）| 更新 `demo-video2.spec.ts`：Explore More 後停頓、分段慢滑全頁到底再回來、加兩件商品；以 ffmpeg fps=15/1280px 重轉 `assets/demo2.gif`（71MB）；`feature/ui-redesign` merge 進 `main` |
